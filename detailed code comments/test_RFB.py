...
parser.add_argument('-v', '--version', default='RFB_vgg',
                    help='RFB_vgg ,RFB_E_vgg or RFB_mobile version.')   # RFBNet模型结构，主干网是vgg、resnet、mobilenet等
...
def test_net(save_folder, net, detector, cuda, testset, transform, max_per_image=300, thresh=0.005):
    ...
    # 这个参数很屌，all_boxes：num_classes x num_images维度，每个元素又是一个5维的numpy，存(x1, x2, y1, y2, score)
    ...
    for i in range(num_images):   # 每张图像的处理
        ...
        _t['im_detect'].tic()
        out = net(x)      # forward pass，结合RFBNet的forward，因为是全卷积网络，所以得到的也是feature map，但包含了loc(其实是offsets) + conf结果
        boxes, scores = detector.forward(out,priors)  # offsets + priors，再辅以decode操作，就可以得到最终的pred bbox + score
        detect_time = _t['im_detect'].toc()
        boxes = boxes[0]
        scores=scores[0]

        boxes *= scale  # bbox float -> int
        ...
        # ----1----- NMS
        for j in range(1, num_classes):    # 对每个类的pred bbox做nms操作，index从0开始，0对应bg
            inds = np.where(scores[:, j] > thresh)[0]   # confidence score满足阈值要求的bbox保存
            ...
            c_dets = np.hstack((c_bboxes, c_scores[:, np.newaxis])).astype(np.float32, copy=False)   # 类似concate操作

            keep = nms(c_dets, 0.45, force_cpu=args.cpu)   # nms
            c_dets = c_dets[keep, :]   # 经过nms操作保存下来高置信度bbox
            all_boxes[j][i] = c_dets   # 注：图像中同一类可能有多个目标，就会对应多个bbox，但都被保存在all_boxes的一个元素内

        # -----2----- top-k抑制
        if max_per_image > 0:
            # [:, -1]对应bbox的score，这个操作就是取出图像中的所有检出bbox的score
            image_scores = np.hstack([all_boxes[j][i][:, -1] for j in range(1,num_classes)])
            if len(image_scores) > max_per_image:  # 检出bbox太多了，只取top-k吧
                image_thresh = np.sort(image_scores)[-max_per_image]   # 卡住top-kth的bbox，作为thres baseline
                for j in range(1, num_classes):
                    keep = np.where(all_boxes[j][i][:, -1] >= image_thresh)[0]   # 对应类 j 下的高score bbox，保留吧
    ...
    print('Evaluating detections')
    testset.evaluate_detections(all_boxes, save_folder)   # 评估函数，可以结合voc_eval.py看看，目标检测番外篇(4)_PascalVOC_metric中也有介绍
...
if __name__ == '__main__':
    ...
    detector = Detect(num_classes,0,cfg)  # 完成forward操作的detector
