...
# bbox格式转换：(x1, x2, y1, y2) -> (cx, cy, w, h)
...
def intersect(box_a, box_b):
    ...
    # 计算 I 中的bbox长、宽，通过min=0规则化，如果两个bbox没有交集，inter中自然有0，此时I就等于0了
...
# 这个也是计算IoU了，用在data_augment.py中，numpy格式的操作
def matrix_iou(a,b):
    """
    ...
    # square I，后面的all函数就是校验两个bbox是否有交集，若无交集，则对应area为0，与torch.clamp((max_xy - min_xy), min=0)目的相同
...
def encode(matched, priors, variances):
    ...
    # 下面两步操作就是对w, h计算log-style的offset
    # match wh / prior wh
...
# 就是我们现在有了在feature map上预定义的密集采样的prior，又有了网络预测输出的loc offset，接下来就是计算bbox reg了
...
def nms(boxes, scores, overlap=0.5, top_k=200):
    ...
    这里面有一个细节，NMS仅用于测试阶段，为什么不用于训练阶段呢？可以评论留言下，我就不解释了，嘿嘿~~~
    Args:
...
# 为什么train的时候不用nms呢～？我的理解是：
# train阶段只对pos样本+3*num(pos)负样本训练，做了ohem了...
# 所以没有必要再在此基础上进行box的抑制..但是test的话，
# 需要把一些iou很小以及detector出太多数量的框给去掉～？
# 和Faster-rcnn不太一样，Faster-rcnn 的 RPN 就得边train边拟制.....

#bingo~
# Faster-rcnn训练细节我有点忘了，但我记得Faster-rcnn训练也做了
# 正负样本比例控制的，也是1:3，但负样本是随机选取的，
# 好像是这样，如果不对继续讨论；